...
ITF_nomagremover.py
等級を含まない観測行を削除したファイルを出力します。トラックレットのうち最初の観測にしか等級を記録しない場合、ファイルサイズを落とすことができます。
python3 ITF_nomagremover.py -input your_itf.txt -output your_itf_nomag.txt
特にDECamの観測に使うと、ファイル数を削減してITF_centercalc.pyが使いやすくなります。もちろんこういった参考の計算を手早くするための加工にすぎないので、間違っても出力ファイルをそのままheliolincで使用してはいけません。
